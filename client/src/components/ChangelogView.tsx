...
export function ChangelogView({ source }: { source: string }): ReactNode {
  ...
  if (intro.length > 0) {
    out.push(
      <div
        key={`intro-${key++}`}
        className="space-y-2 text-sm leading-relaxed opacity-90 [&_a]:underline"
      >
        {intro.map((line, j) => (
          <p key={j}>
            <Inline text={line} />
  ...
  while (i < lines.length) {
    ...
    if (sectionTitle === "[Unreleased]") {
      ...
      if (!hasContent) {
        out.push(
          <p key={`unrel-empty-${key++}`} className="text-sm italic opacity-75">
            Пока без пунктов — новые изменения появятся здесь перед следующим релизом.
          </p>,
        );
      } else {
